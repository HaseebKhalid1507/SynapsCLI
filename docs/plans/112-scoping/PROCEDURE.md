...
- **Take git's clean auto-merges as a phase of their own.** The conflict files are the loud ~20%; the ~90 files git merged cleanly are silent, and hand-phases only touch what the scoping doc names. We lost ~3.6k lines (57 files) until a tests-only pass found ~86 JR unit tests with no home. Add a phase early: `for f in <auto-merged>: cp ref-merge/$f build/$f; re-apply our-side hunks` — then the conflict phases graft onto a complete base.
- **Dropped-test count is the lost-hunk detector.** Per file: `grep -c '^\s*fn [a-z_0-9]*()' <JR> vs <ours> vs <base>`; `ours == base < JR` means JR's `#[cfg(test)]` block never landed — and usually the production code it tests didn't either. Run this after every phase, not at the end.
...
- Landing: `git checkout HEAD -- .` after the `--no-commit` merge does NOT drop untracked additions from the branch — `git diff --cached --name-only` and decide each (we kept JR's docs, dropped the TUI driver files).
