...
- **Run `scripts/merge/symbol-audit.py` after every phase**, not just the dropped-test count. It lists every fn/const/type upstream defines that our copy of the same file lacks. Classify each: superseded (ours is newer), deliberate (replaced by a different design — name the replacement), or LOST. In #112 it found the recall provider-id closure, FORUM_GUIDANCE, and the worker forum-disable — none of which failed a test we'd written.
