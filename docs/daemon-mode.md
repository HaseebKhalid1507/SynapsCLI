...
`synaps` **adopts** it (thin client, fresh session — `SYNAPS_DAEMON_ADOPT=0` opts out);
...
| `SYNAPS_DAEMON_ADOPT=0` | Disable **adopt**: by default a plain `synaps` (no subcommand, no `--attach`; only `--system`/`-s`, `--continue`, `--prompt-manifest`, `--profile` allowed) probes the flock and, if a daemon for that profile is **already running**, runs the thin socket client with `--attach --new` semantics (fresh session; `--continue` still continues) and a boot notice. It never spawns a daemon — that stays `--attach`. No daemon, or `SYNAPS_DAEMON_ADOPT=0`, or any other flag (`--no-extensions`, `--help`, attach modifiers) → the ordinary in-process boot, untouched. |
