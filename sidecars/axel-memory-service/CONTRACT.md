...
* 128 history IDs incl tombstones; <=256MiB serialized history/authorized group; <=16MiB messages+note/segment; <=4096 source messages; note<=8192 bytes. `history_seal` evicts for capacity (bounded ring, docs/specs/archive-size-fit.md): oldest tombstoned rows first (suppression persists in fingerprints), then oldest live rows by created time, physically deleted without tombstone/fingerprint; a seal that cannot fit alone still fails size_limit. `migration_apply` imports never evict. Content fingerprints exclude positional/logical fields, preventing recapture under another logical session. Nested private reasoning/disclosure metadata and nonprojected block grammar are rejected defensively; host remains responsible for text screening.
