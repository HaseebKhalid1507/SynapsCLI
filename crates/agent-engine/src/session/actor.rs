...
impl SessionActor {
    ...
    async fn on_stream_event(&mut self, event: StreamEvent) {
        // #112<-dev merge: the durable context-head checkpoint carries an
        // in-process oneshot ack (ContextHeadReceipt) that cannot cross the
        // daemon wire. The SessionActor is the server and owns durable
        // persistence, so it handles the barrier server-side (match arm below)
        // and never forwards it. All other events forward first, preserving
        // client ordering.
        if !matches!(
            event,
        ...
        match event {
            ...
            StreamEvent::Session(SessionEvent::ContextHeadCheckpoint { receipt, .. }) => {
                // Server-side durable persistence barrier (see on_stream_event
                // top guard). Best-effort save of current session state, then
                // acknowledge so the runtime may proceed. NOTE (#112<-dev merge):
                // adopting the checkpoint's `messages` as the durable head via
                // ContextHeadPersistence::persist is the precise semantics and
                // needs JR review; this preserves liveness in the daemon path.
                self.save().await;
                receipt.complete(Ok(()));
            }
