...
use agent_core::prompt::{
    builtin_orchestration_adapters, compose_orchestration_prompt, ModuleMutability,
    PromptModuleSource, QualifiedModelId, SelectionContext,
};
...
fn builtin_adapters_are_builtin_provider_selected_guidance() {
    ...
    assert_eq!(adapters.len(), 1, "exactly one builtin adapter today");
    let module = &adapters[0];
    assert_eq!(module.id.as_str(), "builtin.codex.subagent-supervision");
