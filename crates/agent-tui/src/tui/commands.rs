...
mod tests {
    ...
    async fn lifecycle_claim_takes_precedence_over_capture_builtin_alias() {
        ...
        let pushed_legacy_error = app.messages.iter().any(|m| matches!(&m.msg, crate::tui::app::ChatMessage::Error(s) if s.contains("no plugin owns /capture")));
    ...
    async fn lifecycle_claim_unknown_subcommand_pushes_error() {
        ...
        let pushed = app.messages.iter().any(|m| matches!(&m.msg, crate::tui::app::ChatMessage::Error(s) if s.contains("unknown /capture subcommand")));
        assert!(pushed);
    }
    ...
    async fn sidecar_toggle_works_when_zero_claims_loaded() {
        ...
        assert!(matches!(action, CommandAction::SidecarToggle { .. }));
        let pushed_err = app.messages.iter().any(|m| matches!(&m.msg, crate::tui::app::ChatMessage::Error(_)));
    ...
    async fn sidecar_toggle_with_one_claim_dispatches_with_hint() {
        ...
        assert!(matches!(action, CommandAction::SidecarToggle { .. }));
        let pushed_hint = app.messages.iter().any(|m| matches!(&m.msg, crate::tui::app::ChatMessage::System(s) if s.contains("try /capture toggle")));
    ...
    async fn sidecar_qualified_unknown_plugin_id_errors() {
        ...
        assert!(matches!(action, CommandAction::None));
        let pushed = app.messages.iter().any(|m| matches!(&m.msg, crate::tui::app::ChatMessage::Error(s) if s.contains("unknown sidecar plugin")));
