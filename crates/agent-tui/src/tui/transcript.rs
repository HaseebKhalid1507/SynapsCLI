...
impl TranscriptStore {
    ...
    /// Remove uncommitted provider previews appended after `start`; retain
    /// human steering/events. Restores the last message at `start - 1` to
    /// `last` when supplied (upstream's rollback snapshot). Called on
    /// `ResponseReset` to undo a failed context-head continuation attempt.
    pub(crate) fn reset_response_preview(&mut self, start: usize, last: Option<ChatMessage>) {
        ...
        self.tool_start_times.retain(|id, _| {
            self.messages.iter().any(|m| {
                matches!(&m.msg, ChatMessage::ToolUseStart { tool_id, .. }
                    | ChatMessage::ToolUse { tool_id, .. } if tool_id == id)
            })
        });
        if self.tool_start_times.is_empty() {
            self.tool_start_time = None;
...
mod response_reset_tests {
    ...
    #[test]
    fn reset_response_preview_basic_truncation() {
        let mut store = TranscriptStore::default();
        store.push_msg(ChatMessage::User("question".into()));
        store.push_msg(ChatMessage::Text("answer round 1".into()));
        ...
        // Only the pre-start messages survive (provider previews removed)
        assert_eq!(store.messages().len(), 2);
        assert_eq!(store.messages()[1].msg.source_text(), "answer round 1");
    }
