...
async fn expired_worker_is_bounded_transport_free_collectible_and_reconcilable() {
    ...
    // authorize() already transitions Dispatched → Starting → Running.
    // Advance to Terminal+Collected so the gate blocks (it only catches
    // Terminal|Collected after the reactive-subagent fix).
    orchestration
