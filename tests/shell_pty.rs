...
use synaps_cli::tools::shell::{session::SessionOpts, SessionManager};
...
async fn test_basic_echo() {
    ...
    let (session_id, _initial, _status) = manager
        .create_session(opts, None, 256 * 1024)
        .await
        .expect("create session");

    let result = manager
        .send_input(&session_id, "echo hello\n", Some(1000), None, 256 * 1024)
        .await
        .expect("send input");
...
async fn test_python_repl() {
    ...
    let result = manager
        .send_input(&session_id, "print(1+1)\n", Some(2000), None, 256 * 1024)
        .await
        .expect("send input");
    ...
    for _ in 0..10 {
        ...
        if let Ok(more) = manager
            .send_input(&session_id, "", Some(500), None, 256 * 1024)
            .await
        {
    ...
    assert!(
        combined.contains('2'),
        "Expected '2' in Python output: '{}'",
        combined
    );

    let _exit_result = manager
        .send_input(&session_id, "exit()\n", Some(1000), None, 256 * 1024)
        .await;
...
async fn test_ctrl_c_interrupt() {
    ...
    let _result = manager
        .send_input(&session_id, "sleep 999\n", Some(500), None, 256 * 1024)
        .await;

    tokio::time::sleep(Duration::from_millis(200)).await;

    // Send Ctrl-C
    let interrupt_result = manager
        .send_input(&session_id, "\x03", Some(1000), None, 256 * 1024)
        .await
        .expect("send ctrl-c");
    assert_eq!(
        interrupt_result.status, "active",
        "Session should still be active after Ctrl-C"
    );

    // Verify session is still responsive
    let test_result = manager
        .send_input(&session_id, "echo test\n", Some(1000), None, 256 * 1024)
        .await
        .expect("send test echo");
    assert!(
        test_result.output.contains("test"),
        "Session should still respond after Ctrl-C"
    );
...
async fn test_ctrl_d_eof() {
    ...
    // Send EOF (Ctrl-D)
    let result = manager
        .send_input(&session_id, "\x04", Some(1000), None, 256 * 1024)
        .await
        .expect("send eof");
    assert_eq!(
        result.status, "exited",
        "Process should have exited after EOF"
    );
...
async fn test_environment_variables() {
    ...
    let result = manager
        .send_input(&session_id, "echo $MY_VAR\n", Some(1000), None, 256 * 1024)
        .await
        .expect("send echo env var");
...
async fn test_max_sessions() {
    ...
    let (session1, _, _) = manager
        .create_session(opts1, None, 256 * 1024)
        .await
        .expect("create first session");
    let (session2, _, _) = manager
        .create_session(opts2, None, 256 * 1024)
        .await
    ...
    manager
        ...
        .await
        .expect("close session1");
    manager
        .close_session(&session2)
        .await
        .expect("close session2");
}
...
async fn test_double_close() {
    ...
    assert_eq!(
        result2.unwrap(),
        "",
        "Second close should return empty string"
    );
}
