...
fn slash_help_palette_query_uses_partial_command_without_slash() {
    assert_eq!(
        synaps_cli::help::prefilter_query_for_slash_command("/mod"),
        Some("mod".to_string())
    );
    assert_eq!(
        synaps_cli::help::prefilter_query_for_slash_command("  /extensions au"),
        Some("extensions au".to_string())
    );
}
...
fn slash_help_palette_query_requires_ambiguous_incomplete_command() {
    assert_eq!(
        synaps_cli::help::prefilter_query_for_slash_command("/"),
        None
    );
    assert_eq!(
        synaps_cli::help::prefilter_query_for_slash_command("hello"),
        None
    );
}
