use synaps_cli::help::{
    builtin_entries, render_entry, render_help, source_display, HelpEntry, HelpExample,
    HelpRegistry, HelpTopicKind,
};
...
fn help_topics_lists_branch_help_and_excludes_command_entries() {
    ...
    assert!(
        rendered.starts_with("Help topics"),
        "topics heading missing:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/help plugins"),
        "topics should include branch help:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/help sessions"),
        "topics should include conceptual branch help:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/help trust"),
        "topics should include conceptual trust branch help:\n{}",
        rendered
    );
    assert!(
        !rendered.contains("/compact"),
        "topics should exclude command entries:\n{}",
        rendered
    );
}
...
fn help_reference_groups_all_entries_including_commands_and_plugins() {
    ...
    assert!(
        rendered.starts_with("Help reference"),
        "reference heading missing:\n{}",
        rendered
    );
    assert!(
        rendered.contains("Core"),
        "reference should group by category:\n{}",
        rendered
    );
    assert!(
        rendered.contains("  /help — Fast paths for finding commands"),
        "reference should include root summary:\n{}",
        rendered
    );
    assert!(
        rendered.contains("  /compact —"),
        "reference should include command entries:\n{}",
        rendered
    );
    assert!(
        rendered.contains("Plugin"),
        "reference should include plugin category:\n{}",
        rendered
    );
    assert!(
        rendered.contains("  /acme:sync — Sync Acme workspace state."),
        "reference should include plugin entries:\n{}",
        rendered
    );
}
...
fn base_help_is_brief_and_points_to_find() {
    ...
    assert!(
        ...
        "base help should stay brief:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/help find"),
        "base help should point to search:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/settings"),
        "base help should include settings:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/plugins"),
        "base help should include plugins:\n{}",
        rendered
    );
    assert!(
        !rendered.contains("/extensions audit"),
        "base help should not dump advanced commands:\n{}",
        rendered
    );
}
...
fn base_help_uses_polished_intro_copy() {
    ...
    let rendered = render_help(&registry, None).expect("base help should render");
    let root = registry
        .entry_by_command("/help")
        .expect("root help entry exists");

    assert_eq!(root.title, "SynapsCLI Help");
    assert_eq!(
        root.summary,
        "Fast paths for finding commands, settings, plugins, models, and diagnostics."
    );
    assert!(
        rendered.contains("Start here. Pick a path or search everything."),
        "missing polished intro:\n{}",
        rendered
    );
    assert!(
        !rendered.contains("Beautiful brief guide"),
        "placeholder copy leaked into rendered help:\n{}",
        rendered
    );
    assert!(
        !root.summary.contains("Beautiful brief guide"),
        "placeholder summary leaked into metadata"
    );
}
...
fn root_help_uses_common_paths_and_guides() {
    ...
    assert!(
        rendered.contains("Common paths"),
        "root should group fast paths:\n{}",
        rendered
    );
    assert!(
        rendered.contains("Guides"),
        "root should include guide links:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/help doctor"),
        "root should include diagnostics path:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/help sessions"),
        "root should link sessions guide:\n{}",
        rendered
    );
    assert!(
        !rendered.contains("Common commands"),
        "phase 1 root should use common paths wording:\n{}",
        rendered
    );
}
...
fn branch_help_renders_specific_topic() {
    ...
    assert!(
        rendered.contains("Plugins"),
        "plugins title missing:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/plugins"),
        "plugins command missing:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/help find"),
        "related discovery missing:\n{}",
        rendered
    );
    assert!(
        !rendered.contains("What would you like to do?"),
        "branch should not duplicate base help:\n{}",
        rendered
    );
}
...
fn phase_one_topics_render_from_help() {
    ...
    for topic in ["sessions", "extensions", "trust", "compact", "chain"] {
        ...
        assert!(
            ...
            "{topic} help should have useful detail:\n{}",
            rendered
        );
    }
}
...
fn phase_one_command_entries_render_directly() {
    ...
    for command in ["/compact", "/chain"] {
        let entry = registry
            .entry_by_command(command)
            .unwrap_or_else(|| panic!("{command} entry exists"));
        let rendered = render_entry(entry);
        assert!(
            rendered.contains(command),
            "{command} detail should show command usage:\n{}",
            rendered
        );
    }
}
...
fn help_entry_deserializes_when_usage_and_examples_are_omitted() {
    ...
    let entry: HelpEntry =
        serde_json::from_str(json).expect("legacy help JSON should still deserialize");
...
fn command_categories_group_common_core_and_session_commands() {
    ...
    assert_eq!(
        registry
            .entry_by_command("/model")
            .map(|entry| entry.category.as_str()),
        Some("Core")
    );
    assert_eq!(
        registry
            .entry_by_command("/settings")
            .map(|entry| entry.category.as_str()),
        Some("Core")
    );
    assert_eq!(
        registry
            .entry_by_command("/clear")
            .map(|entry| entry.category.as_str()),
        Some("Core")
    );
    assert_eq!(
        registry
            .entry_by_command("/sessions")
            .map(|entry| entry.category.as_str()),
        Some("Sessions")
    );
}
...
fn render_entry_includes_usage_and_examples_when_present() {
    ...
    assert!(
        rendered.contains("Examples\n  /example demo    Run with a demo name."),
        "examples section missing:\n{}",
        rendered
    );
    assert!(
        rendered.contains("Related: /help find"),
...
fn builtin_complex_commands_have_usage_and_examples() {
    ...
    for command in ["/compact", "/chain", "/model", "/plugins"] {
        ...
        assert!(
            ...
            "{command} should define examples"
        );
    }
}
...
fn help_drill_down_normalizes_command_topics_before_branch_fallback() {
    ...
    for topic in ["model", "/model"] {
        let rendered = render_help(&registry, Some(topic))
            .unwrap_or_else(|| panic!("{topic} help should render"));
        assert!(
            rendered.starts_with("Model router"),
            "{topic} should render /model command help:\n{}",
            rendered
        );
        assert!(
            rendered.contains("Usage\n  /model [name]"),
            "{topic} should include /model usage:\n{}",
            rendered
        );
        assert!(
            !rendered.starts_with("Models"),
            "{topic} should prefer exact command over models branch:\n{}",
            rendered
        );
    }
}
...
fn help_drill_down_prefers_exact_subcommand_before_branch_fallback() {
    ...
    assert!(
        rendered.starts_with("/extensions audit"),
        "should render exact command entry:\n{}",
        rendered
    );
    assert!(
        rendered.contains("Usage\n  /extensions audit [limit]"),
        "should include audit usage:\n{}",
        rendered
    );
    assert!(
        !rendered.starts_with("Extensions"),
        "should not fall back to extensions branch:\n{}",
        rendered
    );
}
...
fn unknown_help_topic_can_include_closest_suggestions() {
    let registry = HelpRegistry::new(builtin_entries(), Vec::new());
    let rendered =
        render_help(&registry, Some("modle")).expect("unknown help should render fallback");

    assert!(
        rendered.contains("No help topic"),
        "missing unknown-topic message:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/help find"),
        "missing find suggestion:\n{}",
        rendered
    );
    assert!(
        rendered.contains("Closest matches"),
        "missing closest suggestions:\n{}",
        rendered
    );
    assert!(
        rendered.contains("/model"),
        "missing likely /model suggestion:\n{}",
        rendered
    );
}
...
fn unknown_branch_suggests_help_find() {
    let registry = HelpRegistry::new(builtin_entries(), Vec::new());
    let rendered =
        render_help(&registry, Some("wat")).expect("unknown help should render fallback");
...
fn plugin_entries_cannot_override_protected_help_namespace() {
    ...
    assert_ne!(
        registry.entry_by_command("/help").unwrap().title,
        "Hijacked"
    );
...
fn help_find_filters_in_memory_by_command_summary_keywords_and_aliases() {
    ...
    assert!(
        model_matches
            .iter()
            .any(|entry| entry.command == "/help models"),
        "provider should find models help"
    );
...
fn search_ranking_prefix_outranks_alias_keyword_summary_and_body() {
    ...
    assert!(
        commands
            .iter()
            .position(|command| *command == "/summary-hit")
            .unwrap()
            < commands
                .iter()
                .position(|command| *command == "/body-hit")
                .unwrap()
    );
}
...
fn search_alias_match_returns_canonical_entry() {
    ...
    assert_eq!(
        matches.first().map(|entry| entry.command.as_str()),
        Some("/canonical")
    );
}
...
fn plugin_help_entry_with_usage_examples_loads_searches_and_forces_plugin_source() {
    ...
    let registry = HelpRegistry::new(builtin_entries(), plugin_entries);
    let entry = registry
        .entry_by_command("/acme:sync")
        .expect("plugin help entry should load");

    assert!(
        !entry.protected,
        "plugin help entries must not remain protected"
    );
