...
async fn s1_anthropic_success_retry_failure_cancel_records_persist_and_validate() {
    ...
    // Terminal failure: every attempt 500.
    let (url, hits_fail, _) = spawn_stub(Script::AlwaysFailEcho(500)).await;
    std::env::set_var("SYNAPS_ANTHROPIC_BASE_URL", &url);
    let ev = drive_runtime_turn(&rt, "failure fixture", false).await;
    assert!(
        !turn_completed(&ev),
...
/// Cloud invoke (`azure-openai/…` through the remote broker) driven through
/// the REAL `Runtime`: success, failure, cancellation — `CloudProxy`
/// records with honest `wire: None`. RED at d20e03f; GREEN `0d5c46a`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn s1_cloud_invoke_success_failure_cancel_wire_none() {
    ...
    assert!(turn_completed(&ev), "cloud success fixture must complete");
    ...
    assert!(records.iter().any(is_completed));
    assert!(records.iter().any(is_provider_failed));
    assert!(records.iter().any(is_canceled), "{records:#?}");
}

/// Extension-hosted provider (real python sidecar fixture through the real
/// routing gate): success record with `Extension` transport and honest
/// `wire: None`; a request rejected by the gate (unknown model) emits NO
/// attempt record. RED at d20e03f; GREEN `2831ec5`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn s1_extension_provider_success_and_gate_honesty() {
