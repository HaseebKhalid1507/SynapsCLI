...
pub(crate) fn print_status_table(agents: Vec<AgentStatusInfo>) {
    ...
    for agent in agents {
        ...
        println!(
            "{:<15} {:<10} {:<10} {:<10} {:<10} {:<12}",
            agent.name, agent.trigger, agent.status, session, uptime, cost
        );
    }
}
...
pub(crate) fn print_agent_detail(info: AgentStatusInfo) {
    ...
    println!(
        "Cost: ${:.2} today / ${:.2} limit",
        info.cost_today, info.cost_limit
    );
    ...
    if let Ok(json) = serde_json::from_str::<serde_json::Value>(entry) {
        ...
        match log_type {
            "boot" => {
                ...
                Some(format!(
                    "[{}] BOOT session #{} (model: {}, trigger: {})",
                    timestamp, session, model, trigger
                ))
            }
            "tool_start" => {
...
pub(crate) async fn show_logs(
    ...
    if follow {
        ...
        // Poll for new lines
        let mut last_size = std::fs::metadata(&follow_path)
            .map(|m| m.len())
            .unwrap_or(0);
        loop {
